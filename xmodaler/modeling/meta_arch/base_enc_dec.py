...
class BaseEncoderDecoder(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, batched_inputs, use_beam_search=None, output_sents=False, output_semantic=False):
        if use_beam_search is None:
            ...
            return self.greedy_decode(batched_inputs, output_sents, output_semantic)
        else:
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        if kfg.CLIP_SCORE in batched_inputs[0]:
            clip_score = [x[kfg.CLIP_SCORE] for x in batched_inputs]
            clip_score = torch.stack(clip_score)
            ret.update( { kfg.CLIP_SCORE: clip_score } )
        ...
        if kfg.DEMO_STR in batched_inputs[0]:
            demo_strings = [x[kfg.DEMO_STR] for x in batched_inputs]
            if kfg.SEQ_PER_SAMPLE in batched_inputs[0]:
                demo_strings = flat_list_of_lists(demo_strings)
            ret.update( { kfg.DEMO_STR: demo_strings } )

        return ret

    def greedy_decode(self, batched_inputs, output_sents=False, output_semantic=False):
    ...
    def decode_beam_search(self, batched_inputs, output_sents=False, output_semantic=False):
