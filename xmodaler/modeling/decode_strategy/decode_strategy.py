...
from xmodaler.functional import load_vocab, decode_sequence, decode_sequence_bert, read_lines_set
...
class DecodeStrategy(nn.Module, metaclass=ABCMeta):
    ...
    def from_config(cls, cfg):
        ...
        if len(cfg.INFERENCE.SEMANTIC_VOCAB) > 0:
            vocab_sem = read_lines_set(cfg.INFERENCE.SEMANTIC_VOCAB)
        else:
            vocab_sem = None
    ...
    def forward(self, batched_inputs, output_sents, output_semantic, model):
        ret = self._forward(batched_inputs, model)
        if output_sents:
            if self.vocab:
                sents, sents_sem = decode_sequence(self.vocab, ret[kfg.G_SENTS_IDS], self.vocab_sem if output_semantic else None)
            else:
                sents = decode_sequence_bert(self.bert_tokenizer, ret[kfg.G_SENTS_IDS], self.eos_token_id)
            ret.update({ kfg.OUTPUT: sents })

            if output_semantic:
                ret.update({ kfg.OUTPUT_SEM: sents_sem})
