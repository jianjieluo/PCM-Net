...
class MSCoCoDataset:
    ...
    def load_data(self, cfg):
        ...
        if cfg.DATALOADER.CLIP_THRES > 0.0:
            datalist = [item for item in datalist if item.get('clipscore', 1.0) > cfg.DATALOADER.CLIP_THRES]

        if cfg.DATALOADER.DATA_EFF_RATIO > 0 and self.stage == 'train':
            eff_num = int(len(datalist) * cfg.DATALOADER.DATA_EFF_RATIO)
            random.shuffle(datalist)
            datalist = datalist[:eff_num]
    ...
    def __call__(self, dataset_dict):
        ...
        if len(self.feats_folder) > 0:
            ...
            if 'split_id' in dataset_dict: # synthetic image feat
                feat_path = os.path.join(self.feats_folder+'_sp{}'.format(dataset_dict['split_id']), image_id + '.npz')
            else: # GT image feat
            ...
            norm_global_feat = global_feat / (np.linalg.norm(global_feat) + 1e-9)
            if norm_global_feat.shape[0] == 1:
                norm_global_feat = norm_global_feat.reshape(-1)

            ret = { 
                kfg.IDS: image_id, 
                kfg.ATT_FEATS: att_feats,
                kfg.GLOBAL_FEATS: norm_global_feat
            }
        ...
        if 'clipscore' in dataset_dict:
            ret.update({
                kfg.CLIP_SCORE: dataset_dict['clipscore'],
            })
